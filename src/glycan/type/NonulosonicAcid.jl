...
function NeuraminicAcid(s::T; LFP = PyranoseForm, DL = NoDLForm) where T
    i = findfirst(==(0x05 => Acetyl()), s)
    j = findfirst(==(0x05 => Glycolyl()), s)
    if !isnothing(i)
        deleteat!(s, i)
        NacetylneuraminicAcid(isempty(s) ? nothing : s; LFP, DL)
    elseif !isnothing(j)
        deleteat!(s, j)
        NglycolylneuraminicAcid(isempty(s) ? nothing : s; LFP, DL)
    else
        NeuraminicAcid{DL, LFP, T}(s)
    end
end
